...
interface HotelFiltersProps {
  cities: City[];
  /** When true, renders only the inline mobile filter button + bottom sheet (no sidebar) */
  mobileOnly?: boolean;
}
...
export default function HotelFilters({ cities, mobileOnly }: HotelFiltersProps) {
  ...
  // ── mobileOnly mode: render inline button + bottom sheet only ──
  if (mobileOnly) {
    return (
      <>
        <button
  ...
  // ── Default mode: full desktop sidebar ──
  return (
