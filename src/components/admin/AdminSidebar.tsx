...
const menu = [
  ...
  { href: '/admin/app-landing', label: 'إدارة صفحة التطبيق', icon: Smartphone },
...
export default function AdminSidebar({ isFullAdmin }: { isFullAdmin?: boolean }) {
  const pathname = usePathname();

  // Filter menu based on role
  const filteredMenu = menu.filter(item => {
    if (isFullAdmin) return true; // ADMIN sees everything
    
    // BOOKING_STAFF sees only Bookings and Offers
    const allowedForStaff = ['/admin/bookings', '/admin/offers'];
    return allowedForStaff.includes(item.href);
  });
