    ...
    <section className="py-12 sm:py-18 lg:py-24 bg-gradient-to-b from-[#FAF8FD] via-white to-[#F6F4FA] border-t border-neutral-100 overflow-hidden w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* ── 1. Content Column: Clean Typography & Prominent Store Badges (No QR, No Duplicate Cards) ── */}
          <div className="lg:col-span-6 text-center lg:text-start space-y-5 sm:space-y-6">
            ...
            <div className="pt-3 sm:pt-4 flex flex-row items-center justify-center lg:justify-start gap-3 sm:gap-4 w-full sm:w-auto [&>*]:flex-1 sm:[&>*]:flex-initial">
              ...
              <a
                ...
                className="flex items-center justify-center gap-3 px-5 py-3 sm:py-3.5 bg-black hover:bg-neutral-900 text-white rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 shrink-0 border border-neutral-800 group"
              >
                <svg className="w-6 h-6 shrink-0 transition-transform group-hover:scale-110" viewBox="0 0 512 512">
                ...
                <div className="text-start leading-none">
                  <span className="block text-[8px] text-neutral-400 font-bold uppercase mb-0.5">GET IT ON</span>
              ...
              <a
                ...
                className="flex items-center justify-center gap-3 px-5 py-3 sm:py-3.5 bg-black hover:bg-neutral-900 text-white rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 shrink-0 border border-neutral-800 group"
              >
                <svg className="w-6 h-6 fill-white shrink-0 transition-transform group-hover:scale-110" viewBox="0 0 24 24">
                ...
                <div className="text-start leading-none">
                  <span className="block text-[8px] text-neutral-400 font-bold uppercase mb-0.5">Download on the</span>
          ...
          {/* ── 2. TRUE 3D DUAL HARDWARE FLAGSHIPS (Standing on a Flat Surface with Generous Spacing) ── */}
          <div 
            className="lg:col-span-6 flex items-center justify-center pt-4 lg:pt-0 overflow-visible"
            onMouseMove={handleMouseMove}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => { setIsHovered(false); setMousePos({ x: 0, y: 0 }); }}
          >
            {/* Viewport Scaler for Mobile Responsiveness */}
            <div className="transform scale-[0.68] xs:scale-[0.76] sm:scale-[0.85] md:scale-95 lg:scale-100 origin-center py-6">
              <div 
                className="relative w-[380px] sm:w-[460px] lg:w-[500px] h-[480px] sm:h-[510px] lg:h-[530px] flex items-end justify-center"
                style={{
                  perspective: '1500px',
              ...
              >
                ...
                {/* ── Realistic Studio Flat Surface Plane & Shadows ── */}
                {/* Global Ambient Floor Light Plane */}
                <div className="absolute bottom-0 w-full h-16 bg-gradient-to-t from-neutral-300/30 to-transparent rounded-full blur-xl pointer-events-none" />

                {/* S24 Ultra Individual Contact Floor Shadow */}
                <div 
                  className="absolute bottom-2 left-6 sm:left-10 w-[170px] sm:w-[190px] h-10 bg-black/45 rounded-full blur-md transform scale-y-40 -rotate-2 pointer-events-none" 
                />

                {/* iPhone Individual Contact Floor Shadow */}
                <div 
                  className="absolute bottom-2 right-6 sm:right-10 w-[170px] sm:w-[190px] h-10 bg-black/45 rounded-full blur-md transform scale-y-40 rotate-2 pointer-events-none" 
                />

                {/* ── 1. SAMSUNG GALAXY S24 ULTRA (Left Phone: Sharp Titanium Corners, Separated with Gap) ── */}
                <div 
                  className="absolute left-2 sm:left-6 bottom-4 w-[195px] sm:w-[215px] h-[430px] sm:h-[460px] rounded-none bg-gradient-to-b from-[#2E2822] via-[#1A1612] to-[#0A0806] p-[3px] border-[2px] border-[#9E9382] shadow-[-15px_20px_40px_rgba(0,0,0,0.5)] transition-transform duration-300 ease-out z-10"
                  style={{
                    transform: `rotateY(${isHovered ? -14 + mousePos.x * 0.35 : -12}deg) rotateX(${isHovered ? 4 + mousePos.y * 0.35 : 4}deg) rotateZ(-1.5deg)`,
                    transformStyle: 'preserve-3d',
                  }}
                >
                ...
                {/* ── 2. IPHONE 16/17 PRO (Right Phone: Contoured Titanium Bezel, Separated with Gap) ── */}
                <div 
                  className="absolute right-2 sm:right-6 bottom-4 w-[195px] sm:w-[215px] h-[435px] sm:h-[465px] rounded-[38px] sm:rounded-[42px] bg-gradient-to-b from-[#3D352E] via-[#1E1914] to-[#0A0806] p-[3.5px] border-[2.5px] border-[#C8BEB0] shadow-[15px_20px_45px_rgba(0,0,0,0.55)] transition-transform duration-300 ease-out z-20"
                  style={{
                    transform: `rotateY(${isHovered ? 14 + mousePos.x * 0.35 : 12}deg) rotateX(${isHovered ? 4 + mousePos.y * 0.35 : 4}deg) rotateZ(1.5deg)`,
                    transformStyle: 'preserve-3d',
                  }}
