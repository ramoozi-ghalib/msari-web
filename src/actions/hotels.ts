...
/** Phase B migration flag (default ON: API-first with direct fallback on any
 * transport/auth error; only genuine API 404 means not-found).
 * Rollback = MSARI_API_HOTELS_MODE=off. */
 ...
 * Cities come from CityService.getActiveCities (Phase C: API-first + fallback,
 * per-request react cache + 60s persistent cache) — no duplicate /cities fetch.
 */
...
async function getLocalHotelsViaApi(params?: GetLocalHotelsParams): Promise<{
  ...
  const [{ hotels }, apiCities] = await Promise.all([
    ...
    CityService.getActiveCities(100),
  ]);
