...
/**
 * Verifies that the caller has an active session AND satisfies the provided policy.
 * Defaults to checking `Policies.canAccessAdmin`.
 *
...
export async function adminGuard(
  policyCheck: (user: AuthenticatedUser) => boolean = Policies.canAccessAdmin
): Promise<GuardResult> {
  ...
  if (!session?.user) {
    console.warn(`[SECURITY] Mutation Unauthorized: Unauthenticated user attempted an action.`);
    return { ok: false, error: UNAUTHORIZED_RESPONSE };
  }

  // ── Check 2: Does the caller satisfy the specific policy? ─────────────────
  if (!policyCheck(session.user)) {
    console.warn(`[SECURITY] Mutation Forbidden: User ${session.user.id} (${session.user.role}) was denied an action by policy.`);
...
/**
 * Same as adminGuard but requires top-tier user management permissions.
...
export async function superAdminGuard(): Promise<GuardResult> {
  ...
  if (!Policies.canManageUsers(session.user)) {
    return { ok: false, error: FORBIDDEN_RESPONSE };
  }
