...
/**
 * Normalizes a hotel address to the unified "City, Street" format.
 * If the address already starts with the city name, it cleans the separator and formats it as "City, Street".
 * If not, it prepends the city name.
 */
export function normalizeAddress(address: string, cityName: string, separator = '،'): string {
  ...
  if (lowerAddress.startsWith(lowerCity)) {
    const afterCity = trimmedAddress.slice(trimmedCity.length).trim();
    const streetPart = afterCity.replace(/^[-–،,\s]+/, '').trim();
    return streetPart ? `${trimmedCity}${separator} ${streetPart}` : trimmedCity;
  }
  
  return `${trimmedCity}${separator} ${trimmedAddress}`;
}
