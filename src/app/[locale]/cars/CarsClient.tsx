...
export default function CarsClient({ pageContent }: { pageContent?: any }) {
  ...
  const title = pageContent?.hero?.title || 'رحلتك تبدأ بكل راحة وأمان';
  const subtitle = pageContent?.hero?.subtitle || 'احجز سيارة خاصة لتوصيلات المطار، أو التنقل بين المدن في رحلتك عبر اليمن.';
  ...
  const fleet = (pageContent?.fleet && pageContent.fleet.length > 0) ? pageContent.fleet : DEFAULT_FLEET;
    ...
    <div className="bg-[#f8f8fa] min-h-screen pb-20">
      ...
      <div className="relative pt-24 pb-32 lg:pb-40 overflow-hidden">
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-8 lg:pt-16">
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-extrabold mb-3 leading-tight drop-shadow-lg" style={{ color: '#ffffff' }}>
            {title}
          </h1>
          <p className="text-white/90 text-xs sm:text-sm lg:text-base max-w-2xl mx-auto mb-8 drop-shadow-md">
            {subtitle}
          </p>
        </div>
      </div>
