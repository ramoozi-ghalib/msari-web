...
export default function DestinationsManagementClient({ initialCities }: Props) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        setSaveError(res.error?.message || 'حدث خطأ ما');
      }
    });
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. Get presigned URL
      const authRes = await getUploadPresignedUrl(file.name, file.size, file.type, 'destinations');
      ...
      if (authRes.success && authRes.signedUrl && authRes.publicUrl) {
        ...
        if (uploadRes.ok) {
          setFormData(prev => ({ ...prev, imageUrl: authRes.publicUrl }));
        } else {
