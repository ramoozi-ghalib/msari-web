...
const mockUsers: User[] = [
  ...
  { id: '2', name: 'سارة محمد', email: 'sara@msari.net', role: 'BOOKING_STAFF', status: 'active', lastLogin: '2025-06-15 10:15 AM' },
  { id: '3', name: 'خالد يحيى', email: 'khaled@msari.net', role: 'BOOKING_STAFF', status: 'inactive', lastLogin: '2025-05-20 02:00 PM' },
];
...
export default function UsersManagement() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    role: 'BOOKING_STAFF' as 'ADMIN' | 'BOOKING_STAFF',
  ...
  const handleOpenEdit = (user: User) => {
    ...
    setFormData({
      ...
      role: (user.role === 'ADMIN' || user.role === 'BOOKING_STAFF') ? user.role : 'BOOKING_STAFF',
  ...
  const handleDelete = (userTarget: User) => {
    if (Policies.isFullAdmin(userTarget)) {
    ...
    if (confirm('هل أنت متأكد من حذف هذا المشرف بصفة نهائية؟ لن يتمكن من تسجيل الدخول مجدداً.')) {
      setUsers(users.filter(u => u.id !== userTarget.id));
    }
  };

  const toggleStatus = (userTarget: User) => {
    if (Policies.isFullAdmin(userTarget)) {
      alert('لا يمكن إيقاف حساب المدير العام السوبر.');
      return;
    }
    setUsers(users.map(u => u.id === userTarget.id ? { ...u, status: userTarget.status === 'active' ? 'inactive' : 'active' } : u));
  };
                ...
                <tr key={user.id} className="hover:bg-neutral-50 transition-colors">
                  ...
                  <td className="py-4 px-6 not-italic">
                    <button 
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${Policies.isFullAdmin(user) ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'} ${user.status === 'active' ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100'}`}>
                  ...
                  <td className="py-4 px-6 text-end not-italic">
                    <div className="flex justify-end gap-2">
                      ...
                      <button 
                        ...
                        className={`p-2 border border-neutral-200 bg-white shadow-sm rounded-lg transition-colors ${Policies.isFullAdmin(user) ? 'text-neutral-300 cursor-not-allowed' : 'text-red-600 hover:bg-red-50'}`} 
                        disabled={Policies.isFullAdmin(user)}
                        title="حذف الحساب">
                        <Trash2 size={16} />
                      </button>
                    </div>
            ...
            <div className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-2">
                  ...
                  <select 
                    value={formData.role}
                    onChange={(e) => setFormData({...formData, role: e.target.value as 'ADMIN'|'BOOKING_STAFF'})}
                    className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl focus:outline-none focus:border-[#23096e] focus:bg-white transition-colors"
                  >
                    <option value="BOOKING_STAFF">مشرف المنصة</option>
