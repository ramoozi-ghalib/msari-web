...
import { getHotelForAdmin } from '@/actions/hotels';
import { getUploadPresignedUrl } from '@/actions/storage';
// ─── Helpers ────────────────────────────────────────────────────────────────
...
const SPACE_DETAILS = [
  { id: 's1', name: 'عدد الغرف', icon: 'DoorOpen' },
  { id: 's2', name: 'عدد الأسرّه', icon: 'BedDouble' },
  { id: 's3', name: 'عدد الحمامات', icon: 'Bath' },
  { id: 's4', name: 'عدد الأشخاص', icon: 'Users' },
];
...
export default function HotelsManagementClient({ initialHotels, initialCities, dbAmenities }: Props) {
  ...
  const hotelAmenitiesFromDB = dbAmenities.filter(a => a.category === 'HOTEL_AMENITIES');
  const roomAmenitiesFromDB  = dbAmenities.filter(a => a.category === 'ROOM_FEATURES');
  ...
  const openEdit = async (hotel: Hotel) => {
    // Show modal immediately, then load full hotel data (including rooms)
    setIsModalOpen(true);
    setLoadingRooms(true);
    const fullHotel = await getHotelForAdmin(hotel.id);
    setLoadingRooms(false);
    const targetHotel = fullHotel ?? hotel; // fallback to original if fetch fails
    setEditingHotel(targetHotel);
    setFormData({
      name: targetHotel.name,
      nameEn: targetHotel.nameEn || '',
      cityId: targetHotel.cityId || '',
      stars: targetHotel.stars,
      priceFrom: targetHotel.priceFrom,
      discountPercentage: targetHotel.discount?.percentage || 0,
      isFeatured: targetHotel.isFeatured || false,
      isActive: targetHotel.isActive !== false,
      address: targetHotel.address || '',
      description: targetHotel.description || '',
      checkInTime: '14:00',
      checkOutTime: '12:00',
      policyAr: targetHotel.policyAr || '',
      images: targetHotel.images || [],
      mapUrl: targetHotel.mapUrl || '',
    });
    // Pre-select amenity IDs
    const hotelAmenityIds = targetHotel.amenities?.map((a: any) => a.id) || [];
    setSelectedHotelAmenities(hotelAmenityIds);
    setActiveTab('basic');
    setRoomForm(null);
    setSaveMsg('');
    if (!fullHotel) {
      alert('فشل جلب تفاصيل الفندق مع الغرف. سيتم عرض البيانات المتاحة فقط.');
    }
  };
  ...
  const handleHotelImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      // 1. طلب تصريح رفع (Presigned URL)
      const authRes = await getUploadPresignedUrl(file.name, file.size, file.type, 'hotels');
      
      if (authRes.success && authRes.signedUrl && authRes.publicUrl) {
        // 2. رفع الملف مباشرة إلى Supabase
        const uploadRes = await fetch(authRes.signedUrl, {
          method: 'PUT',
          body: file,
        ...
        if (uploadRes.ok) {
          setFormData(prev => ({
            ...prev,
            images: [...prev.images, authRes.publicUrl]
          }));
        } else {
          console.error('Upload failed:', await uploadRes.text());
          alert('فشل رفع الصورة إلى مزود التخزين.');
        }
      } else {
        alert('فشل في تصريح الرفع: ' + (authRes as any).error);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleRoomImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. طلب تصريح رفع (Presigned URL)
      const authRes = await getUploadPresignedUrl(file.name, file.size, file.type, 'rooms');
      ...
      try {
        ...
        if (res.success && hotelId) {
          ...
          if (discRes.success) {
            ...
            const errStr = typeof discRes.error === 'string' ? discRes.error : (discRes.error as any)?.message || '';
            setSaveMsg('❌ حدث خطأ أثناء حفظ الخصم: ' + errStr);
          }
        } else if (!res.success) {
          const errStr = typeof res.error === 'string' ? res.error : (res.error as any)?.message || '';
          setSaveMsg('❌ حدث خطأ أثناء الحفظ: ' + errStr);
        } else {
          // Unexpected case (e.g., success but missing hotelId)
