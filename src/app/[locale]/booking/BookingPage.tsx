 ...
 *
 * [FIX C-3] إزالة useEffect + fetch() كلياً.
 * بيانات الفندق تُستقبَل كـ prop جاهز من Server Component الأب.
 *
 * [FIX M-8] السعر يُعرَض من previewBookingPrice() server action
 * قبل الإتمام — لا حساب client-side يعتمد على URL params.
 */
...
/**
 * [FIX C-3] الـ hotel يأتي كـ prop — لا useEffect ولا fetch()
 */
export default function BookingPage({ city, hotel, hotelId, roomId, roomName, checkIn, checkOut, guests, nights, bankAccounts = [] }: BookingPageProps) {
  ...
  // [FIX M-8] السعر من server-action — لا حساب client-side
  ...
  async function handleConfirm(method: string) {
    ...
    const result = await createBooking({
      ...
      checkOut:      toISODatetime(checkOut),
      guests,
      paymentMethod: PAYMENT_METHOD_MAP[method] ?? 'BANK_TRANSFER',
    ...
    <div className="min-h-screen bg-[#f8f8fa] py-28">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-8">
          ...
          <p className="text-neutral-400 text-sm">راجع الحجز ثم أكمل بياناتك لإرسال طلب الحجز</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {step === 'review' && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-100 space-y-5">
                <div>
                  <h2 className="text-lg font-black text-neutral-900">مراجعة تفاصيل الحجز</h2>
                  <p className="text-sm text-neutral-500 mt-1">هذه الخطوة للتأكد قبل إدخال بيانات الضيف.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="rounded-xl border border-neutral-100 bg-neutral-50 p-4">
                    ...
                    <p className="font-bold text-neutral-900">{roomName || 'سيتم اختيار الغرفة المتاحة في الفندق'}</p>
                  </div>
                  <div className="rounded-xl border border-neutral-100 bg-neutral-50 p-4">
                    <p className="text-neutral-400 text-xs mb-1">تواريخ الإقامة</p>
                    <p className="font-bold text-neutral-900">{checkIn} ← {checkOut}</p>
                  </div>
                  <div className="rounded-xl border border-neutral-100 bg-neutral-50 p-4">
                    <p className="text-neutral-400 text-xs mb-1">الضيوف</p>
                    <p className="font-bold text-neutral-900">{guests} ضيف · {pricePreview?.nights ?? nights} {nights === 1 ? 'ليلة' : 'ليالٍ'}</p>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-[#23096e]/20 bg-[#23096e]/5 p-4">
                  ...
                  <p className="text-2xl font-black text-[#23096e] mt-1">
                    {loadingPrice
                      ? '...'
                      : formatPrice(pricePreview?.finalTotal ?? 0)}
                  </p>
                </div>
                ...
                <button
                  ...
                  className="w-full text-white font-black py-4 rounded-xl hover:opacity-90 transition-all shadow-md"
          ...
          <div>
            <BookingSummaryBox
              ...
              // [FIX M-8] السعر من server — لا حساب client-side
              serverTotal={loadingPrice ? undefined : pricePreview?.finalTotal}
              serverCurrency={pricePreview?.currency}
