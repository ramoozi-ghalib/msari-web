...
export const SetHotelDiscountSchema = z.object({
  ...
  // [FIX M-6] تواريخ الخصم اختيارية — إذا لم تُحدَّد تُستخدم القيم الافتراضية (اليوم + سنة)
  validFrom:  z.string().datetime({ message: 'تاريخ البداية غير صالح' }).optional(),
  validTo:    z.string().datetime({ message: 'تاريخ الانتهاء غير صالح' }).optional(),
}).strict();
