# QA Evidence Matrix — F1 Source Reconciliation (Final)
...
|---|---------|----------------|-----------------|-------|
| 1 | `reserveBookingNumber` callable exists in production | **PROVEN** | firebase-debug.log lines 82, 117 | Active callable, `entryPoint="reserveBookingNumber"`, `httpsTrigger`, status=ACTIVE |
...
| 7 | No branch/tag/stash contains `reserveBookingNumber` export | **PROVEN** | `git branch -a`, `git tag`, `git stash list` | Only `master` branch, one tag `v2c-p0`, empty stash |
| 8 | No local checkout has the source | **PROVEN** | Full text search `D:\Dev`, `D:\projects` | Only references in mobile caller, docs, debug symbols, deploy logs |
| 9 | Deployed hash `2220afbd` ≠ current repo HEAD `475f788` | **PROVEN** | Deploy log vs `git rev-parse HEAD` | Different trees |
| 10 | Deployed build `341433fa` ≠ any local build | **PROVEN** | Deploy log vs local builds | No matching build locally |
| 11 | Mobile caller exists and depends on callable | **PROVEN** | `lib/data/services/hotel_booking_service.dart:262-264` | `_reserveBookingNumber()` calls `httpsCallable('reserveBookingNumber')` |
| 12 | Mobile fallback generator has zero call sites | **PROVEN** | `booking_number_generator.dart` zero call sites | Legacy generator unused |
| 13 | Source Upload URL expired/inaccessible | **PROVEN** | HTTP HEAD/GET → 403 Forbidden | Signed URL expired (deploy 2026-09-01) |
...
| Item | Status | Notes |
|---|---|---|
| Credential exposure in logs | CLEAR | No credentials/secrets in any searched artifacts |
...
| Credential leakage in logs | CLEAR | No secrets in debug logs or artifacts |
